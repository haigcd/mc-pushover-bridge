...
class Notifier {
    ...
    constructor() {
        ...
        axios.defaults.headers.common['Authorization'] = `Bearer ${env.smartthings.bearer_token}`;
    }
    ...
    private async ding(): Promise<void> {
        const resp = await axios.post(env.smartthings.endpoint, JSON.stringify(env.smartthings.payload));
    }
    ...
    async run(): Promise<void> {
        ...
        const addedPlayers = newPlayerList
            .filter(p => !this.currentPlayers.includes(p));

        if (addedPlayers.length > 0) {
            this.logger.info("Added these players: ", addedPlayers);
            this.pushover.send(`${addedPlayers.length} player(s) joined`, addedPlayers.toString());
            // await this.ding();
        }

        const removedPlayers = this.currentPlayers
            .filter(p => !newPlayerList.includes(p));

        if (removedPlayers.length > 0) {
            this.logger.info("Removed these players: ", removedPlayers);
            this.pushover.send(`${removedPlayers.length} player(s) left`, removedPlayers.toString());
        }
